...
import qualified ChessBoard                    as CB
...
main = Hspec.hspec $ do
      ...
      PPText.renderStrict
        . PP.layoutPretty
            (PP.defaultLayoutOptions
              { PP.layoutPageWidth = PP.AvailablePerLine n 1
              }
            )
  ...
  describe "gisting floaty things" $ do
    ...
    describe "Double" $ do
      for_ tests $ \(args, input :: Double, expected) -> do
        let desc = concat
              [show args, " / ", show input, " === ", show expected]
  ...
  countingTests "Bunch of tests" $ \numberedTest -> do
    ...
    numberedTest $ do
      gistF80
          (\c ->
            c { Gist.gistElem = Gist.FnGister (\_ () -> "_") }
              :: Gist.ConfigList ()
          )
          [()]
        `shouldBe` "[_]"
  ...
  countingTests "chessboard" $ \numberedTest -> do
    ...
    numberedTest $ do
      gistF80
          (let
               gLastMoved = Gist.defaultConfGisterF $ \c ->
                 c { Gist.ConfigMaybe.showConstructors = True }
               gPiece = Gist.defaultConfGisterF $ \c ->
